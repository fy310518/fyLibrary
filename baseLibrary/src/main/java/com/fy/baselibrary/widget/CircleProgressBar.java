...
/**
 * describe： 自定义圆形进度条 view
 * Created by fangs on 2018/12/21 9:55.
 */
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.circleProgressBar_firstColor) {
                ...
                circleWidth = ta.getDimensionPixelSize(attr, (int) TypedValue.applyDimension(
                        TypedValue.COMPLEX_UNIT_DIP, 0, getResources().getDisplayMetrics())); // 默认圆弧宽度为6dp
            }
        }
        ta.recycle();
...
//        drawText(canvas, center, radius);
    ...
    private void drawCircle(Canvas canvas, int center, int radius) {
        ...
        if (circleWidth != 0){
            ...
            circlePaint.setColor(Color.parseColor("#00000000")); // 设置底部圆的颜色，这里使用第一种颜色
            circlePaint.setStyle(Paint.Style.FILL);
            canvas.drawCircle(center, center, radius, circlePaint); // 画底部的空心圆

            circlePaint.setColor(Color.parseColor("#ffffff"));
            canvas.drawArc(oval, -90, alphaAngle, true, circlePaint);
        }
    }
